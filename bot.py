...
WEBHOOK_DEPENSES = os.environ.get("WEBHOOK_DEPENSES", "")
WEBHOOK_REVENUS = os.environ.get("WEBHOOK_REVENUS", "")
...
def envoyer_vers_sheets(payload, type_op):
    url = WEBHOOK_REVENUS if type_op == "revenu" else WEBHOOK_DEPENSES
    if not url:
        return "Webhook non configure"
    try:
...
async def enregistrer(update, user_id):
    ...
    status = envoyer_vers_sheets(payload, type_op)
    if type_op == "revenu":
        message = (
            "Revenu enregistre !\n\n"
